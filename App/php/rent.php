...
if (mysqli_num_rows($result) == 0) {
	$zapytanie0 = "SELECT czy_sprawny FROM rower WHERE id_roweru=$id_roweru";
	$wynik = mysqli_query($conn, $zapytanie0);
	$row = mysqli_fetch_assoc($wynik);   
	$zmienna = $row[czy_sprawny];
	if($zmienna == 1){
		$zapytanie1 = "UPDATE uzytkownik SET wypozyczenia=wypozyczenia + 1 WHERE login='$login';";
		$zapytanie2  = "INSERT INTO wypozyczenia (login, id_rower, czas) VALUES ('$login', '$id_roweru', CURRENT_TIMESTAMP);";
		...
		echo "Niestety rower, który chcesz wypożyczyć jest niesprawny. Przepraszamy za problem!";
	}
}else{
